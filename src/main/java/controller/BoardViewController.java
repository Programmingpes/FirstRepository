...
public class BoardViewController implements Controller {
	...
	public ModelAndView execute(HttpServletRequest request, HttpServletResponse response)
		...
		//게시글에 해당하는 댓글을 읽어옴
		ArrayList<BoardCommentDTO> list 
		= (ArrayList<BoardCommentDTO>) BoardService.getInstance().selectCommentList(bno);
		request.setAttribute("list", list);
		
		ArrayList<FileDTO> filelist
		= (ArrayList<FileDTO>) BoardService.getInstance().selectFileList(bno);
